...
# The MCP server (backend/mcp_server.py) is stdio-only and deliberately not served over HTTP here:
# its Streamable-HTTP transport holds long-lived connections, which a serverless host bills as
# active compute. Run it locally with `python -m backend.mcp_server`.
