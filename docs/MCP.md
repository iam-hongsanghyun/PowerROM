...
| `get_country_profile` | Full technology profile for a country (capex/opex, fuel price, heat rate, emission factor, capacity factor, per-technology ramp rates, storage, discount rate, sources). |
| `calculate_lcoe` | Price a fleet: hourly dispatch → system LCOE, emission intensity, curtailment, unserved energy, import dependency, per-tech LCOE, realised mix. Exposes **every policy lever**: carbon price, EV load, min/max CF limits, per-generator ramp limits (`ramp_up`/`ramp_down`, fraction of nameplate per hour — these override the per-technology defaults carried in the profile), manual merit order, storage (power + duration), capacity expansion, an RPS target + penalty, ITC/PTC clean-energy subsidies, a fuel-import tariff, demand-shape controls, ensemble/weather-year settings, and profile overrides. Returns a compact summary by default; opt into the **full output** with `include_dispatch` (per-generator metric bands), `include_ldc` (full 8760-pt Load-Duration-Curve), `include_hourly` (full 8760-h generation), `include_inputs` (every resolved setting + the merged profile), or `full=True` for all of it. |
